...
const TransDetPrdTmpController = () => {

  const addTransDetPrdTmp = async (req, res) => {
    let { body } = req;
      try {
        let insert = await TransDetPrdTmp.create({
...
const getTransDetPrdTmp= async (req, res) => {
    try {
      let header = await TransDetPrdTmp.findAll(
        {
          limit :25,
        }
        );
...
module.exports = TransDetPrdTmpController;
